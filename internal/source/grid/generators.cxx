...
bool Place_Piezo_Generator(int X, int Y) {
	...
	Data_L.Data_Grid[X][Y][Power_Cap] = 5;
...
bool Place_Bio_Generator(int X, int Y) {
    if (Check_Clearance(X, Y, 3, 3)) {
		...
		Data_L.Animation_Grid[X][Y][1] = 45;
		Data.Wiring_Grid[X][Y] = 1;
		Data_L.Data_Grid[X][Y][Power_Cap] = 9500;
		...
		Data_L.Data_Grid[Nodes[0].X][Nodes[0].Y][1] = 4;
...
bool Place_Geo_Well(int X, int Y) {
		...
		if (Check_Clearance(X, Y, 2, 3)) {
			...
			for (int Counter = 0; Counter < 2; Counter++) {
				if (Interface.Placing_Rotation == Counter * 2) {
					Data_L.Data_Grid[X + Counter][Y][Fluid_Cap] = 10;
					Data_L.Data_Grid[X + Counter][Y + 2][Fluid_Cap] = 10;
					...
					Data_L.Data_Grid[X][Y][5] = std::abs((Counter * 80) - 28);
				}
			}
		}
	} else {
		if (Check_Clearance(X, Y, 3, 2)) {
			...
			for (int Counter = 0; Counter < 2; Counter++) {
				if (Interface.Placing_Rotation == (Counter * 2) + 1) {
					Data_L.Data_Grid[X][Y + Counter][Fluid_Cap] = 10;
					Data_L.Data_Grid[X + 2][Y + Counter][Fluid_Cap] = 10;
					...
					Data_L.Data_Grid[X][Y][6] = std::abs((Counter * 80) - 28);
				}
			}
		}
	}
	if (Placed) {
		...
		Data_L.Data_Grid[X][Y][Power_Cap] = 3000;
...
bool Place_Turbine_Input(int X, int Y) {
	...
	if (Placed) {
		...
		Data_L.Data_Grid[Node.X][Node.Y][Fluid_Cap] = LDE_LARGECAP;

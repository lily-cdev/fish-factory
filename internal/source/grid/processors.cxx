...
bool Place_Filtration_Plant(int X, int Y) {
	if (Check_Clearance(X, Y, 2, 3)) {
		...
		Data_L.Data_Grid[X][Y][Power_Cap] = 95;
		Data_L.Data_Grid[X][Y][4] = 5;
		Data_L.Data_Grid[X][Y][5] = 12;
		Data_L.Data_Grid[X][Y][6] = 100;
		Data_L.Data_Grid[X][Y + 1][Fluid_Cap] = 16;
		Data_L.Data_Grid[X + 1][Y][Fluid_Cap] = 12;
		for (int Counter = 1; Counter < 3; Counter++) {
			Data_L.Data_Grid[X + 1][Y + Counter][Fluid_Cap] = 4;
...
bool Place_Distillery(int X, int Y) {
    if (Check_Clearance(X, Y, 2, 2)) {
		...
		if (Interface.Placing_Rotation == 0) {
			...
			Data.Visual_Grid[X][Y] = Interface.Placing_Rotation + 116;
		}
		Data_L.Data_Grid[X][Y][5] = 24;
		Data_L.Data_Grid[X][Y][6] = 56;
		if (Interface.Placing_Rotation == 2 || Interface.Placing_Rotation == 3) {
			Data_L.Data_Grid[X][Y][5] = 56;
		}
		if (Interface.Placing_Rotation == 1 || Interface.Placing_Rotation == 2) {
			Data_L.Data_Grid[X][Y][6] = 24;
		}
		Data.Wiring_Grid[X][Y] = 0;
		Data_L.Data_Grid[X][Y][Power_Cap] = 2500;
		...
		for (int Counter = 0; Counter < Nodes.size(); Counter++) {
			...
			Data_L.Data_Grid[Nodes[Counter].X][Nodes[Counter].Y][Fluid_Cap] = 6;
		...
		for (int Counter = 0; Counter < Nodes.size(); Counter++) {
			...
			Data_L.Data_Grid[Nodes[Counter].X][Nodes[Counter].Y][Fluid_Cap] = 4;
			Data.Plumbing_Grid[Nodes[Counter].X][Nodes[Counter].Y] =
	...
	if (evn_i(Interface.Placing_Rotation)) {
		if (Check_Clearance(X, Y, 3, 2)) {
			...
			for (int Counter = 0; Counter < 3; Counter++) {
				Data_L.Data_Grid[X + Counter][static_cast<int>(
			...
			Data_L.Data_Grid[X + 1][Y - static_cast<int>
			...
			if (Interface.Placing_Rotation == 0) {
				Data_L.Data_Grid[X][Y][5] = 44;
				Data_L.Data_Grid[X][Y][6] = 67;
			} else {
				Data_L.Data_Grid[X][Y][5] = 76;
				Data_L.Data_Grid[X][Y][6] = 13;
			}
			Placed = true;
		}
	} else {
		if (Check_Clearance(X, Y, 2, 3)) {
			Fill_Clearance(LDE_INVALID, X, Y, 2, 3);
			for (int Counter = 0; Counter < 3; Counter++) {
				Data_L.Data_Grid[X - static_cast<int>(
			...
			Data_L.Data_Grid[X + static_cast<int>((Interface
			...
			if (Interface.Placing_Rotation == 1) {
				Data_L.Data_Grid[X][Y][5] = 13;
				Data_L.Data_Grid[X][Y][6] = 44;
			} else {
				Data_L.Data_Grid[X][Y][5] = 67;
				Data_L.Data_Grid[X][Y][6] = 76;
			}
			Placed = true;
		}							
	}
	if (Placed) {
		Data_L.Data_Grid[X][Y][Power_Cap] = 70000;
		Data.Wiring_Grid[X][Y] = 0;
		if (Interface.Placing_Rotation == 0) {
...
bool Place_Fluid_Mixer(int X, int Y) {
	if (Check_Clearance(X, Y, 3, 3)) {
		...
		for (int Counter = 0; Counter < Nodes.size(); Counter++) {
			...
			Data_L.Data_Grid[Nodes[Counter].X][Nodes[Counter].Y][Fluid_Cap] = 24;
		...
		for (int Counter = 0; Counter < Nodes.size(); Counter++) {
			...
			Orientation -= Interface.Placing_Rotation;
			Orientation = ((Orientation + 3) & 3) + 1;
			Data_L.Data_Grid[Nodes[Counter].X][Nodes[Counter].Y][Fluid_Cap] = 36;
		...
		Data_L.Animation_Grid[X][Y][1] = 0;
		Data_L.Data_Grid[X][Y][Power_Cap] = 800;
		Data.Wiring_Grid[X][Y] = 0;
		Data_L.Data_Grid[X][Y][5] = 60;
		Data_L.Data_Grid[X][Y][6] = 60;
	} else {
		return false;
	}
	return true;

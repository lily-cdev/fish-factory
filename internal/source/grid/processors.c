...
bool Place_Distillery(int X, int Y) {
    if (Check_Clearance(X, Y, 2, 2)) {
		...
		Node Nodes;
		Return_Nodes(Nodes, X, Y, Interface.Placing_Rotation, Preconfigurations.D_Inputs);
		...
		Return_Nodes(Nodes, X, Y, Interface.Placing_Rotation, Preconfigurations.D_Outputs);
		for (int Counter = 0; Counter < Nodes.Length; Counter++) {
...
bool Place_Electrolytic_Cell(int X, int Y) {
	...
	if (evn_i(Interface.Placing_Rotation)) {
		if (Check_Clearance(X, Y, 3, 2)) {
			...
			Data.Plumbing_Grid[X + 1][Y - (int)(Interface.Placing_Rotation * 0.5) + 1] =
				Down - Interface.Placing_Rotation;
			Data.Settings_Grid[X + 1][Y - (int)(Interface.Placing_Rotation * 0.5) + 1][0] = 1;
			if (Interface.Placing_Rotation == 0) {
...
bool Place_Fluid_Mixer(int X, int Y) {
	if (Check_Clearance(X, Y, 3, 3)) {
		...
		Node Nodes;
		Return_Nodes(Nodes, X, Y, Interface.Placing_Rotation, Preconfigurations.FM_Inputs);
		for (int Counter = 0; Counter < Nodes.Length; Counter++) {
			int Orientation = (int)(floor(Counter / 3.0f) * 2) + Left;
			Orientation -= Interface.Placing_Rotation;
			Orientation = ((Orientation + 3) & 3) + 1;
			Data.Data_Grid[Nodes.Data[Counter].X][Nodes.Data[Counter].Y][Fluid_Cap] = 24;
		...
		Return_Nodes(Nodes, X, Y, Interface.Placing_Rotation, Preconfigurations.FM_Outputs);
		for (int Counter = 0; Counter < Nodes.Length; Counter++) {
			int Orientation = (Counter * 2) + Up;
			Orientation -= Interface.Placing_Rotation;
			Orientation = ((Orientation + 3) & 3) + 1;
			Data.Data_Grid[Nodes.Data[Counter].X][Nodes.Data[Counter].Y][Fluid_Cap] = 36;

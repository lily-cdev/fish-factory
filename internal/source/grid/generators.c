...
bool Place_Bio_Generator(int X, int Y) {
    if (Check_Clearance(X, Y, 3, 3)) {
		...
		Node Nodes;
		Return_Nodes(Nodes, X, Y, Interface.Placing_Rotation, Preconfigurations.BG_Inputs);
		...
		if (Check_Clearance(X, Y, 2, 3)) {
			...
			for (int Counter = 0; Counter < 2; Counter++) {
				if (Interface.Placing_Rotation == Counter * 2) {
					...
					Data.Data_Grid[X][Y][5] = abs((Counter * 80) - 28);
				}
			}
		}
	} else {
		if (Check_Clearance(X, Y, 3, 2)) {
			...
			for (int Counter = 0; Counter < 2; Counter++) {
				if (Interface.Placing_Rotation == (Counter * 2) + 1) {
					...
					Data.Data_Grid[X][Y][6] = abs((Counter * 80) - 28);
				}
			}
		}
	}
...
bool Place_Turbine_Input(int X, int Y) {
	...
	if (Placed) {
		...
		Node Nodes;
		Return_Nodes(Nodes, X, Y, Interface.Placing_Rotation, Preconfigurations.STI_Inputs);
...
bool Place_Turbine_Output(int X, int Y) {
	if (Check_Clearance(X, Y, 2, 2)) {
		...
		Node Nodes;
		Return_Nodes(Nodes, X, Y, Interface.Placing_Rotation, Preconfigurations.STO_Outputs);
		Data.Data_Grid[Nodes.Data[0].X][Nodes.Data[0].Y][Fluid_Cap] = LDE_LARGECAP;

			...
			for (int C1 = 0; C1 < ptlen(Step.Placement_Locations);
				...
				if (Step.Type == 3) {
					for (int C2 = 0; C2 < Pipes.Length; C2++) {
						if (Pipes.Data[C2].X1 == Step.Placement_Locations[C1].X &&
							Pipes.Data[C2].Y1 == Step.Placement_Locations[C1].Y &&
							Pipes.Data[C2].X2 == Step.Placement_Locations[C1 + 1].X &&
							Pipes.Data[C2].Y2 == Step.Placement_Locations[C1 + 1].Y) {
							Incomplete = false;
						}
					}
				} else {
					for (int C2 = 0; C2 < Wires.Length; C2++) {
						if (Wires.Data[C2].X1 == Step.Placement_Locations[C1].X &&
							Wires.Data[C2].Y1 == Step.Placement_Locations[C1].Y &&
							Wires.Data[C2].X2 == Step.Placement_Locations[C1 + 1].X &&
							Wires.Data[C2].Y2 == Step.Placement_Locations[C1 + 1].Y) {
							Incomplete = false;
						}
					}
				}

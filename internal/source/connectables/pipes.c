		...
		for (int Row = 0; Row < LDE_GRIDSIZE; Row++) {
			...
			if (Detect_Mouse_Collision(Rects.Tile_1x1)) {
				...
				if (Pipes.Length > 0 && !Pipes.Data[End].Filled) {
					...
					if ((Column < LDE_GRIDSIZE && Pipes.Data[End].X1 == Column + 1) || (Column >= 0 && Pipes.Data[End].X1 ==
						Column - 1)) {
						Is_Adjacent = true;
					...
					if (Is_Pipe_Adjacent && Data.Plumbing_Grid[Column][Row] != LDE_INVALID && (Data.Settings_Grid[Column][Row][
						0] == 0 || Data.Settings_Grid[Column][Row][0] == 1)) {

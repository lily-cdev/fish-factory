...
void Render_Help(Point Pos) {
		...
		if (Temporary.Tutorial_Step == ktn_invalid) {
			if (Data.CMD_Placed) {
				for (int C1 = 1; C1 < Textures.Tutorials.Length; C1++) {
					...
					Render_Button(&Textures.Tutorials.Data[C1], &Rects.Tutorials.Data[C1], (UI_Link){ (C1 == 1) ? Gen_Tutorial : Farm_Tutorial },
				...
				Render_Button(&Textures.Tutorials.Data[0], &Rects.Tutorials.Data[0], (UI_Link){ CMD_Tutorial }, Colors.Cherry_Blossom);
			}
		} else {
	...
	case 2:
		{
			char Flavor[] = "The following cheat items are intentionally unbalanced.| |You can use them for troubleshooting, bug-hunting, or just playing"
				" around.";
			Render_Rich_Text(F_Subtext, Flavor, (Point){ 20, 60 }, false, false);
		}
		for (int C1 = 0; C1 < 3; C1++) {
			Render_Button(&Textures.Cheats.Data[C1], &Rects.Cheats.Data[C1], (UI_Link){ Spawn_Cheat, .Param.Integer = C1 }, Colors.Cherry_Blossom);
		}
		break;
	default:
		break;

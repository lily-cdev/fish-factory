	...
	if (Interface.Subtab > 0) {
		if (Interface.Subtab < 9) {
			...
			for (int C1 = 0; C1 < Subcategories; C1++) {
				Render_Button(&Textures.Subcategories.Data[Interface.Subtab - 1].Data[C1], &Rects
					.Subcategories.Data[Interface.Subtab - 1].Data[C1], C1 + 3, Colors.Cherry_Blossom);
			}
			for (int C1 = 0; C1 < intlen(Metadata.Item_Labels[Interface.Subtab - 1]); C1++) {
				Render_Button(&Textures.Item_Labels.Data[Interface.Subtab - 1].Data[C1], &Rects
					.Item_Labels.Data[Interface.Subtab - 1].Data[C1], C1 + Subcategories + 3, Colors.Cherry_Blossom);
			}
		} else {
			for (int C1 = 0; C1 < intlen(Metadata.Subcontents[Interface.Subtab - 9]); C1++) {
				Render_Button(&Textures.Subcontents.Data[Interface.Subtab - 9].Data[C1], &Rects
					.Subcontents.Data[Interface.Subtab - 9].Data[C1], C1 + 3, Colors.Cherry_Blossom);
			}
		}
		...
		for (int C1 = 0; C1 < intlen(Quirk_Stack); C1++) {
			...
			if (Detect_Mouse_Collision(Quirk_Rectangle)) {
				SDL_GetTextureSize(Textures.Quirk_Label.Data[Quirk_Stack[C1]], &Label_Rects[C1].w,
					&Label_Rects[C1].h);
		...
		for (int C1 = 0; C1 < intlen(Quirk_Stack); C1++) {
			...
			SDL_FRect Temporary_Rectangle = Buffer_Rectangle(Label_Rects[C1], 4, 1);
		...
		for (int C1 = 0; C1 < LDE_CATEGORIES; C1++) {
			Render_Button(&Textures.Categories.Data[C1], &Rects.Categories.Data[C1], C1 + 3,

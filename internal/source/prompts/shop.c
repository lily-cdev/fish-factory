...
void Render_Shop(Point Pos) {
	...
	if (Interface.Subtab > 0) {
		if (Interface.Subtab <= LDE_CATEGORIES) {
			...
			for (int C1 = 0; C1 < veclen(Metadata.Item_Labels[Interface.Subtab - 1]); C1++) {
			...
			for (int C1 = 0; C1 < veclen(Metadata.Subcontents[Interface.Subtab - LDE_CATEGORIES - 1]); C1++) {
		...
		float Multiplier = scale_f(120.0f) / Icon_Rectangle.w;
		Icon_Rectangle = (SDL_FRect){
			scale_f(103.0f) - (Icon_Rectangle.w * Multiplier * 0.5f),
			scale_f(24.0f),
		...
		SDL_Surface* Desc_Surface = TTF_RenderText_Blended_Wrapped(Fonts.Faces[F_Subtext], Interface.Item->Desc, 0,
			Colors.Abyss_Black, scale_f(180));
		SDL_Texture* Desc_Texture = Surface_To_Texture(Desc_Surface);
		SDL_FRect Desc_Rect = { scale_f(16.0f), scale_f(Offset + 40.0f), (float)Desc_Texture->w, (float)Desc_Texture->h };
		Render_Texture(Desc_Texture, &Desc_Rect);
		SDL_DestroySurface(Desc_Surface);
		free_texture(Desc_Texture);
		for (int C1 = 0; C1 < intlen(Quirk_Stack); C1++) {

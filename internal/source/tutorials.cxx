...
void Render_Tutorial() {
	if (Temporary.Tutorial_Step > LDE_INVALID) {
		SDL_FRect Bounding_Rectangle = { (static_cast<float>(120 - Interface.Camera_X) + Temporary.Tutorial_Offset[0]) *
			Settings.Screen_Size, (static_cast<float>(120 - Interface.Camera_Y) + Temporary.Tutorial_Offset[1]) *
			Settings.Screen_Size, static_cast<float>(Temporary.Tutorial_Size[0] * Settings.Screen_Size),
			static_cast<float>(Temporary.Tutorial_Size[1] * Settings.Screen_Size) };
		Temporary.ID_Query.push_back(0);
		Temporary.Query.push_back(Bounding_Rectangle);
		Temporary.Color_Query.push_back(Colors.Pure_White);
		...
		if (Tutorial_Stack[Temporary.Tutorial_Step].Type == 0) {
			std::string Keycore = SDL_GetKeyName(Keybinds.Keybind_List[Tutorial_Stack[Temporary.Tutorial_Step].Hotkey]);
			...
			for (int Counter = 0; Counter < Tutorial_Stack[Temporary.Tutorial_Step]
				...
				if ((Data.Visual_Grid[Tutorial_Stack[Temporary.Tutorial_Step]
					...
					SDL_FRect Outline_Rectangle = { static_cast<float>((Tutorial_Stack
						[Temporary.Tutorial_Step].Placement_Locations[Counter][0] * 40 * Settings.Screen_Size) -
						(Interface.Camera_X * Settings.Screen_Size)), static_cast<float>((Tutorial_Stack
						[Temporary.Tutorial_Step].Placement_Locations[Counter][1] * 40 *	Settings.Screen_Size) -
						(Interface.Camera_Y * Settings.Screen_Size)), static_cast<float>(X * 40 *
						Settings.Screen_Size), static_cast<float>(Y * 40 * Settings.Screen_Size) };
					Temporary.ID_Query.push_back(0);
					Temporary.Query.push_back(Outline_Rectangle);
					Temporary.Color_Query.push_back(Colors.Carnage_Pink);
					Remaining++;
				}
			}
			std::string Suffix = "s";
			...
			if (Tutorial_Stack[Temporary.Tutorial_Step].Item == 0 &&
				...
				if (Tutorial_Stack[Temporary.Tutorial_Step].ID_Override) {
					Subcore = Metadata.Machine_Names[Tutorial_Stack[Temporary.Tutorial_Step].Item];
				} else {
					Subcore = Metadata.Machine_Names[Visual_To_ID(
			...
			for (int Counter1 = 0; Counter1 < Tutorial_Stack
				...
				if (Incomplete) {
					SDL_FRect Temporary_Rectangle = { static_cast<float>((Tutorial_Stack
						...
						40) + ((20 - Interface.Camera_X) * Settings.Screen_Size)), static_cast<float>((Tutorial_Stack
						[Temporary.Tutorial_Step].Placement_Locations[Counter1][1] * Settings.Screen_Size * 40) +
						((20 - Interface.Camera_Y) * Settings.Screen_Size)), static_cast<float>((Tutorial_Stack
						...
						40) + ((20 - Interface.Camera_Y) * Settings.Screen_Size)) };
					Temporary.Query.push_back(Temporary_Rectangle);
					Temporary.ID_Query.push_back(1);
					Temporary.Color_Query.push_back(Colors.Carnage_Pink);
					Remaining++;
				}
			}
			...
			Position = Interface.Camera_Y;
			switch (Tutorial_Stack[Temporary.Tutorial_Step].Hotkey) {
			case 1:
				Keybind = 2;
				Direction = "left";
				Position = Interface.Camera_X;
				break;
			case 3:
				Keybind = 3;
				Direction = "right";
		...
		if (Detect_Mouse_Collision({ 170.0f * Settings.Screen_Size, 20.0f * Settings.Screen_Size,
			...
			Render_Dynamic_Text(Fonts.Subtext_Font, "This will " + Tutorial_Stack[Temporary.Tutorial_Step].Context +
				".", Colors.Abyss_Black, LDE_INVALID, 26);
		} else {

...
void Load_Modular(const char* Path, Texture_Array* Yield, int Size) {
	...
	for (int C1 = 0; C1 < 6; C1++) {
		Subtextures.Data[C1] = SDL_GenerateTexture(Core.Renderer, LDE_TILESIZE * Size, LDE_TILESIZE * Size);
...
void Load_Rotational(const char* Path, Texture_Array* Yield) {
	...
	for (int C1 = 1; C1 < 4; C1++) {
		Yield->Data[C1] = SDL_GenerateTexture(Core.Renderer, Maximum, Maximum);
...
void Load_Animated_Rotational(const char* Path, Texture2_Array* Yield, int Height, bool Inverted, int* Rotationals) {
	...
	for (int C1 = 0; C1 < 4; C1++) {
		...
		for (int C2 = 0; C2 < Subyield.Length; C2++) {
			...
			Yield->Data[C1].Data[C2] = SDL_GenerateTexture(Core.Renderer, Max, Max);

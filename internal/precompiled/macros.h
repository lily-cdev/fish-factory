...
#define ktn_memcpy(Victim, Source, Size) do { if ((Source) == NULL) { memset(Victim, 0, Size); } else { memcpy(Victim, Source, Size); } } while (0)
#define ktn_intlen(Victim) ({ int Yield = 0; while ((Victim)[Yield] != ktn_terminator) { Yield++; } Yield; })
#define ktn_ptlen(Victim) ({ int Yield = 0; while ((Victim)[Yield].X != ktn_terminator || (Victim)[Yield].Y != ktn_terminator) { Yield++; } Yield; })
...
#define ktn_charcat(Victim, A, B) do { int Length = strlen(Victim); if (Length < B) { (Victim)[Length] = (A); (Victim)[Length + 1] = '\0'; } } while (0)
#define ktn_free_texture(Victim) do { if (Victim) { SDL_DestroyTexture(Victim); Victim = NULL; } } while (0)
#define ktn_load_bmp(Victim, Path) do { (Victim) = Load_BMP(Path); if ((Victim) == NULL) { char Carrier[128]; snprintf(Carrier, sizeof(Carrier), \
    "could not load image \"%s\"", Path); ktn_jump(I_No_Image, Carrier); } } while (0)
...
#ifdef __unix__
    #define ktn_stricmp(A, B) (strcasecmp((A), (B)) == 0)
#else
    #define ktn_stricmp(A, B) (_stricmp((A), (B)) == 0)
#endif

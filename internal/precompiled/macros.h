...
#define memcpy_c(Victim, Source, Size) do { if ((Source) == NULL) { memset(Victim, 0, Size); } else { memcpy(Victim, Source, Size); } } while (0)
...
#define charcat(Victim, A, B) do { int Length = strlen(Victim); if (Length < B) { (Victim)[Length] = (A); (Victim)[Length + 1] = '\0'; } } while(0)
